...
void SSP0Init( void )
{
  ...
  /* Control register to turn power on */
  /* |= is there to set only bit 21 */
  LPC_SC->PCONP |= (0x1<<21);

  /* Further divider is needed on SSP0 clock. Using default divided by 4 */
  /* in datasheet, refer page 58, table 41 */
  /* Below operation is to clear those two bits by negation */
  /* bit 11 10 is clock for SSP0, that is why 03 that is 11 in binary */
  LPC_SC->PCLKSEL1 &= ~(0x3<<10);

  /* P0.15~0.18 as SSP0 */
  /* In table 80, 8.5.1, the port nums are from 12 to 19 for SSEL1 to MOSI1 */
